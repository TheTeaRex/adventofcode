...
class Cave:
    def __init__(
        self, lines: List[str], has_floor: bool = False, offset_lowest: int = 0
    ) -> None:
    ...
    def sand_drops(self) -> Tuple[int, int]:
        ...
        while True:
            ...
            while (
                next_spot not in self.rocks
                and next_spot not in self.sands
                and next_spot[1] <= self.cave_lowest_point
            ):
                sand = next_spot
                next_spot = (sand[0], sand[1] + 1)
