...
class Solution:
    def __init__(self, filename: str):
        ...
        rooms = self.maps_rooms(text)
        rooms = self.maps_dist_from_room_to_room(rooms)
        self.part1 = self.solution(rooms["AA"], 30, rooms, 1)
        self.part2 = self.solution(rooms["AA"], 26, rooms, 2)
    ...
    def maps_rooms(self, lines: List[str]) -> Dict[str, Room]:
        """
        ...
        for line in lines:
            m = re.match(
                r"Valve (?P<name>\w+) has flow rate=(?P<rate>\d+); tunnels? leads? to valves? (?P<to_rooms>((\S+)( )?)+)",  # noqa: E501
                line,
            )
            temp = {}
    ...
    def maps_dist_from_room_to_room(self, rooms: Dict[str, Room]) -> Dict[str, Room]:
        """
    ...
    def solution(  # noqa: C901
        ...
        def dfs(
            room: Room, time: int, bitmask: int, cur_value: int, max_val: int
        ) -> int:
    ...
    def prints_rooms(self, rooms: Dict[str, Room]) -> None:
        """
